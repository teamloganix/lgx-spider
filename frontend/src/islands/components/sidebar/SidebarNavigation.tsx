...
import { Tooltip, TooltipContent, TooltipTrigger } from '@islands/components/ui/tooltip';
import { Badge } from '@islands/components/ui/badge';
import { ChevronDown } from 'lucide-react';
import { navigationItems, type NavigationItem } from '../data/navigation-config';
...
export default function SidebarNavigation({
  ...
  const isSubItemCurrent = (subItemId: string): boolean => subItemId === currentPage;
  ...
  const renderNavigationItem = (item: NavigationItem) => {
    ...
    const isExpanded =
      expandedItems[item.id] !== undefined ? expandedItems[item.id] : subItemActive;
        ...
        <Tooltip>
          ...
          {isCollapsed && (
            <TooltipContent side="right">
              <p>
                {item.label} {showBadge && `(${cartItemCount})`}
              </p>
            </TooltipContent>
          )}
        </Tooltip>
                  ...
                  <TooltipTrigger asChild>
                    <button
                      onClick={() => {
                        if (subItem.path) {
                          const baseUrl = import.meta.env.BASE_URL || '/';
                          const np = subItem.path.startsWith('/')
                            ? subItem.path
                            : `/${subItem.path}`;
                          const fullPath =
                            baseUrl === '/' ? np : `${baseUrl.replace(/\/$/, '')}${np}`;
                          window.location.href = fullPath;
